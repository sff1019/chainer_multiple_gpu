...
class ParallelMLP(chainer.Chain):
    def __init__(self, n_units, n_out, gpu0, gpu1, gpu2, gpu3):
        ...
        super(ParallelMLPm, self).__init__()
        with self.init_scope():
    ...
    def forward(self, x):
        ...
        h0 = F.relu(z0 + F.copy(z1, gpu_0) + F.copy(z2, gpu_0), F.copy(z3, gpu_0))
        h1 = F.relu(z1 + F.copy(z0, gpu_1) + F.copy(z2, gpu_1), F.copy(z3, gpu_1))
        h2 = F.relu(z2 + F.copy(z0, gpu_2) + F.copy(z1, gpu_2), F.copy(z3, gpu_2))
        h3 = F.relu(z3 + F.copy(z0, gpu_3) + F.copy(z1, gpu_3), F.copy(z2, gpu_3))
        ...
        y = y0 + F.copy(y1, gpu_0) + F.copy(y2, gpu_0) + F.copy(y3, gpu_0)
